...
public class SellerStoreController {
    ...
    public ResponseEntity<?> updateStore(@PathVariable String storeId,
        ...
        if (changes.containsKey("address")) {
            safe.put("address", changes.get("address"));
        }
        ...
        return ResponseEntity.ok(Map.of("success", true, "store", mapStoreWithAddress(updated)));
    }
    ...
    private Map<String, Object> mapStoreWithAddress(Store s) {
        Map<String, Object> m = new HashMap<>();
        m.put("id", s.getId());
        m.put("name", s.getName());
        m.put("ownerId", s.getOwnerId());
        ...
        m.put("orderingDisabled", s.getOrderingDisabled());
        m.put("closedReason", s.getClosedReason());
        m.put("closedUntil", s.getClosedUntil());
        ...
        addr.put("full", s.getFullAddress());
        addr.put("shopNo", s.getShopNo());
        addr.put("building", s.getBuilding());
        addr.put("street", s.getStreet());
        addr.put("area", s.getArea());
        addr.put("city", s.getCity());
        addr.put("pincode", s.getPincode());
        addr.put("lat", s.getLat());
        addr.put("lng", s.getLng());
        m.put("address", addr);
    ...
    public ResponseEntity<?> getDashboardStats() {
        ...
        List<Store> stores = factoryProvider.getSellerFactory(tenant).stores().list(null, null, null);
        if (stores.isEmpty()) {
        ...
        List<Order> orders = factoryProvider.getSellerFactory(tenant).orders().list(store.getId(), null, null, null, null, null);
        ...
        long pendingOrders = orders.stream()
                .filter(o -> "placed".equalsIgnoreCase(o.getStatus()) || "preparing".equalsIgnoreCase(o.getStatus()))
                .count();
    ...
    public ResponseEntity<?> getProfile() {
        ...
        UserEntity user = userRepository.findById(principal.getUserId()).orElse(null);
        ...
        if (store != null) {
            response.put("store_name", store.getName());
            
            Map<String, Object> addr = new HashMap<>();
            addr.put("full", store.getFullAddress());
            addr.put("shopNo", store.getShopNo());
            addr.put("building", store.getBuilding());
            addr.put("street", store.getStreet());
            addr.put("area", store.getArea());
            addr.put("city", store.getCity());
            addr.put("pincode", store.getPincode());
            addr.put("lat", store.getLat());
            addr.put("lng", store.getLng());
            
            response.put("address", addr);
        } else {
             response.put("store_name", null);
    ...
    @PostMapping("/store")
    public ResponseEntity<?> createDefaultStore(@RequestBody Map<String, Object> payload) {
        String tenant = com.bharatshop.tenant.TenantContext.getTenant();
        var principal = UserPrincipal.current();
        
        // Check if store already exists for this seller
        List<Store> existing = factoryProvider.getSellerFactory(tenant).stores().list(null, null, null);
        if (!existing.isEmpty()) {
             throw new com.bharatshop.error.ApiException(org.springframework.http.HttpStatus.CONFLICT, "store_exists", "Seller already has a store");
        }
        
        Store s = new Store();
        s.setName((String) payload.get("name"));
        s.setArea((String) payload.get("area"));
        s.setCity((String) payload.get("city"));
        s.setCategory((String) payload.get("category"));
        s.setOwnerId(principal.getUserId());
        s.setOwnerPhone(resolvePhone(principal.getUserId()));
        
        if (payload.containsKey("address")) {
             Object addrObj = payload.get("address");
             if (addrObj instanceof Map) {
                 Map<?, ?> addr = (Map<?, ?>) addrObj;
                 if (addr.containsKey("full")) s.setFullAddress((String) addr.get("full"));
                 if (addr.containsKey("shopNo")) s.setShopNo((String) addr.get("shopNo"));
                 if (addr.containsKey("building")) s.setBuilding((String) addr.get("building"));
                 if (addr.containsKey("street")) s.setStreet((String) addr.get("street"));
                 if (addr.containsKey("city")) s.setCity((String) addr.get("city"));
                 if (addr.containsKey("pincode")) s.setPincode((String) addr.get("pincode"));
                 if (addr.containsKey("lat")) {
                     Object lat = addr.get("lat");
                     if (lat instanceof Number) s.setLat(((Number) lat).doubleValue());
                 }
                 if (addr.containsKey("lng")) {
                     Object lng = addr.get("lng");
                     if (lng instanceof Number) s.setLng(((Number) lng).doubleValue());
                 }
             }
        }
        ...
        return ResponseEntity.ok(Map.of("success", true, "store", mapStoreWithAddress(created)));
    }

    @PatchMapping("/store")
    public ResponseEntity<?> updateDefaultStore(@RequestBody Map<String, Object> changes) {
        String tenant = com.bharatshop.tenant.TenantContext.getTenant();
        List<Store> stores = factoryProvider.getSellerFactory(tenant).stores().list(null, null, null);
        if (stores.isEmpty()) {
            // Upsert: Create if not exists
            return createDefaultStore(changes);
        }
        Store store = stores.get(0);
