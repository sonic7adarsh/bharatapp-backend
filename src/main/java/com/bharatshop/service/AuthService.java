...
public class AuthService {
    ...
    public Map<String, Object> sendOtp(String phone) {
        return sendOtp(phone, null, false);
    }

    public Map<String, Object> sendOtp(String phone, String role, boolean isRegistration) {
        log.info("Sending OTP to phone={} role={} isRegistration={}", phone, role, isRegistration);
        if (phone == null || phone.isBlank()) return Map.of("success", false, "error", "phone_required");

        // Smart Checks for Role-based flows (Industry Level UX)
        if (role != null && !role.isBlank()) {
            String tenant = TenantContext.getTenant();
            if (tenant == null || tenant.isBlank()) tenant = "default";
            
            java.util.List<UserEntity> users = userRepository.findByTenantIdAndPhone(tenant, phone);
            UserEntity user = users.isEmpty() ? null : users.get(0);
            String targetRole = canonicalRole(role);
            
            if (isRegistration) {
                // Register Flow: Fail if user exists AND has role
                if (user != null) {
                    java.util.List<String> roles = userRoleService.getUserRoles(user.getId());
                    if (roles.contains(targetRole)) {
                         throw new com.bharatshop.error.BadRequestException("Account already exists. Please login.");
                    }
                }
            } else {
                // Login Flow: Fail if user missing OR doesn't have role
                if (user == null) {
                     throw new com.bharatshop.error.BadRequestException("Account not found. Please register.");
                } else {
                    java.util.List<String> roles = userRoleService.getUserRoles(user.getId());
                    if (!roles.contains(targetRole)) {
                         throw new com.bharatshop.error.BadRequestException("Account not authorized for " + targetRole + ". Please register.");
                    }
                }
            }
        }
    ...
    /**
     * Unified OTP verification with role support.
     * Handles both login and registration (implicit or explicit).
     * - If user missing: Create user, assign CUSTOMER and requested role.
     * - If user exists:
     *   - If role assigned: Login.
     *   - If role missing:
     *     - If isRegistration=true: Add role, Login.
     *     - If isRegistration=false: Error (Require registration).
     */
    public Session verifyOtpAndEnableRole(String phone, String otp, String role, boolean isRegistration) {
        log.info("Verifying OTP for phone={} role={} isRegistration={}", phone, role, isRegistration);
        OtpInfo info = otpsByPhone.get(phone);
        if (info == null) {
            log.warn("OTP verification failed: no OTP for phone={}", phone);
            return null;
        ...
        // Validate tenant
        String tenant = TenantContext.getTenant();
        if (tenant == null || tenant.isBlank()) throw new IllegalStateException("Tenant missing");
        String tenantId = tenant.trim();
        ...
        if (users.isEmpty()) {
            entity = createTenantUserWithPhone(tenantId, phone); // Creates with CUSTOMER role
            isNewUser = true;
        } else {
            entity = users.get(0);
        }
        ...
        // Handle Role Logic
        boolean hasRole = userRoleService.getUserRoles(entity.getId()).contains(canonicalRole);
        
        if (!hasRole) {
            if (isNewUser || isRegistration) {
                userRoleService.addRoleToUser(entity.getId(), canonicalRole);
                log.info("Added role={} to user={}", canonicalRole, entity.getId());
            } else {
                throw new com.bharatshop.error.ApiException(
                    org.springframework.http.HttpStatus.FORBIDDEN,
                    "ROLE_NOT_ASSIGNED",
                    "User is not registered as " + canonicalRole + ". Please register first.",
                    java.util.Map.of("requestedRole", canonicalRole)
                );
            }
        }
        
        // Switch active role and create session
        userRoleService.switchUserRole(entity.getId(), canonicalRole);
        return createSession(fromEntity(entity));
    }
